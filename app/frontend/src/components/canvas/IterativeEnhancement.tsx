...
export function IterativeEnhancement({
  ...
  const [, setInitialNodeCount] = useState(0);
  const [, setInitialEdgeCount] = useState(0);
  const [, setCurrentNodeCount] = useState(0);
  const [, setCurrentEdgeCount] = useState(0);
  ...
  const startIteration = async (startFromNodeId?: string) => {
    ...
    try {
      ...
      const { data: projectData } = await pronghornApi.rpc("get_project_with_token", {
        p_project_id: projectId,
        p_token: shareToken || null,
      });

      const response = await invokeEdgeFunction("orchestrate-agents", {
        projectId,
        shareToken,
        agentFlow: {
        ...
        selectedModel: projectData?.selected_model || "gpt-4o",
  ...
  agentFlowNodes.find((n) => n.id === editingAgentNodeId);
  ...
  const handleSaveAsArtifact = async () => {
    try {
      ...
      await invokeEdgeFunction("upload-artifact-image", {
        projectId,
        shareToken,
