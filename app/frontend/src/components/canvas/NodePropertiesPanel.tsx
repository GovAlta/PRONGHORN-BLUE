...
export function NodePropertiesPanel({
  ...
  const loadDropdownData = async () => {
    ...
    if (node.data.type === "REQUIREMENT") {
      if (token) {
        const { data } = await pronghornApi.rpc("get_requirements_with_token", {
    ...
    if (node.data.type === "STANDARD") {
      const { data } = await pronghornApi.from("standards").select("id, code, title").order("code");
      setStandards(data || []);
    }

    // Tech stacks table is not project-scoped, direct query allowed
    if (node.data.type === "TECH_STACK") {
      const { data } = await pronghornApi.from("tech_stacks").select("id, name").order("name");
      setTechStacks(data || []);
    }
  };
  ...
  const handleStandardChange = async (standardId: string) => {
    ...
    const { data } = await pronghornApi.from("standards").select("code, title").eq("id", standardId).single();
  ...
  const handleTechStackChange = async (techStackId: string) => {
    ...
    const { data } = await pronghornApi.from("tech_stacks").select("name").eq("id", techStackId).single();
