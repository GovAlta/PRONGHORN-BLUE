...
export function InfographicDialog({ projectId, shareToken, open, onOpenChange }: InfographicDialogProps) {
  ...
  useRealtimeArtifacts(projectId, shareToken, true);
  ...
  const generateInfographic = async () => {
    ...
    try {
      ...
      const { data, error } = await pronghornApi.functions.invoke("generate-image", {
        body: { 
          selectedContent,
          generationType: selectedGenerationType,
          style: selectedStyle,
  ...
  const saveImageAsArtifact = async (image: GeneratedImage) => {
    ...
    try {
      ...
      const { data, error } = await pronghornApi.functions.invoke("upload-artifact-image", {
        body: {
          projectId,
          shareToken,
