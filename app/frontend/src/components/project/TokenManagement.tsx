...
export function TokenManagement({ projectId, shareToken }: TokenManagementProps) {
  ...
  const { data: tokens, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await pronghornApi.rpc("get_project_tokens_with_token" as any, {
  ...
  useEffect(() => {
    ...
    if (!tokenExists) {
      ...
      if (ownerToken) {
        ...
        Promise.resolve(pronghornApi.rpc("set_share_token", { token: ownerToken.token })).catch(console.error);
      }
    }
  ...
  const createTokenMutation = useMutation({
    mutationFn: async () => {
      const { data, error } = await pronghornApi.rpc("create_project_token_with_token" as any, {
        p_project_id: projectId,
        p_token: activeToken || shareToken || null,
        p_role: newTokenRole,
  ...
  const deleteTokenMutation = useMutation({
    mutationFn: async (tokenId: string) => {
      const { error } = await pronghornApi.rpc("delete_project_token_with_token" as any, {
        p_token_id: tokenId,
        p_token: activeToken || shareToken || null,
      });
  ...
  const [, setRollingTokenValue] = useState<string | null>(null);
  ...
  const rollTokenMutation = useMutation({
    mutationFn: async ({ tokenId, oldTokenValue }: { tokenId: string; oldTokenValue: string }) => {
      ...
      const { data, error } = await pronghornApi.rpc("roll_project_token_with_token" as any, {
        p_token_id: tokenId,
        p_token: activeToken || shareToken || null,
      });
    ...
    onSuccess: ({ newToken, oldToken }) => {
      ...
      if (oldToken === currentToken) {
        ...
        Promise.resolve(pronghornApi.rpc("set_share_token", { token: newToken })).catch(console.error);
      ...
      const url = `${window.location.origin}/project/${projectId}/requirements/t/${newToken}`;
  ...
  const copyTokenUrl = (token: string) => {
    const url = `${window.location.origin}/project/${projectId}/requirements/t/${token}`;
    navigator.clipboard.writeText(url);
  ...
  const copyCurrentAccessUrl = () => {
    ...
    const url = `${window.location.origin}/project/${projectId}/settings/t/${displayToken}`;
    navigator.clipboard.writeText(url);
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              <Input 
                readOnly 
                value={showMyToken 
                  ? `${window.location.origin}/project/${projectId}/settings/t/${displayToken}`
                  : `${window.location.origin}/project/${projectId}/settings/t/••••••••••••`
                }
