...
import { FileText, MessageSquare, ListTree, BookOpen, Layers, Network, Info, CheckSquare, Square, Loader2, FileCode, Database } from "lucide-react";
...
export function ProjectSelector({
  ...
  const loadProjectStandards = async () => {
    try {
      ...
      const { data: projectStandards } = await pronghornApi.rpc(
      ...
      // Get all categories
      const { data: categoriesData } = await pronghornApi
        .from("standard_categories")
        .select("*")
        .order("order_index");

      // Get all standards
      const { data: standardsData } = await pronghornApi
        .from("standards")
        .select("*")
  ...
  const loadProjectTechStacks = async () => {
    try {
      ...
      const { data: projectTechStacks } = await pronghornApi.rpc(
      ...
      const { data: linkedStacks, error } = await pronghornApi
        .from("tech_stacks")
      ...
      const { data: parentStacks } = await pronghornApi
        .from("tech_stacks")
        .select("*")
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (selectedArtifacts.size > 0) {
        const { data, error } = await pronghornApi.rpc("get_artifacts_with_token", {
          p_project_id: projectId,
          p_token: shareToken || null
        });
        if (error) throw error;
      ...
      if (selectedChats.size > 0) {
        const { data: sessionsData, error } = await pronghornApi.rpc("get_chat_sessions_with_token", {
          p_project_id: projectId,
          p_token: shareToken || null
        });
        if (error) throw error;
        ...
        const sessionsWithMessages = await Promise.all(
          selectedSessions.map(async (session: any) => {
            const { data: messagesData } = await pronghornApi.rpc("get_chat_messages_with_token", {
      ...
      if (selectedTechStacks.size > 0) {
        const { data: tsData } = await pronghornApi
          .from("tech_stacks")
          .select("*")
      ...
      if (selectedNodes.size > 0) {
        const { data, error } = await pronghornApi.rpc("get_canvas_nodes_with_token", {
          p_project_id: projectId,
          p_token: shareToken || null
        });
        if (error) throw error;
      ...
      if (selectedEdges.size > 0) {
        const { data, error } = await pronghornApi.rpc("get_canvas_edges_with_token", {
          p_project_id: projectId,
          p_token: shareToken || null
        });
        if (error) throw error;
      ...
      if (selectedLayers.size > 0) {
        const { data, error } = await pronghornApi.rpc("get_canvas_layers_with_token", {
          p_project_id: projectId,
          p_token: shareToken || null
        });
        if (error) throw error;
        ...
        if (layerNodeIds.size > 0 && canvasNodes.length > 0) {
          const allNodesData = await pronghornApi.rpc("get_canvas_nodes_with_token", {
            p_project_id: projectId,
            p_token: shareToken || null
          });
      ...
      if (selectedFiles.size > 0) {
        // Get Prime repo
        const { data: repos } = await pronghornApi.rpc("get_project_repos_with_token", {
          p_project_id: projectId,
          p_token: shareToken
        });
        ...
        if (primeRepo) {
          // Get committed files
          const { data: committedFiles } = await pronghornApi.rpc("get_project_files_with_token", {
            p_project_id: projectId,
            p_token: shareToken
          });

          // Get staged changes
          const { data: stagedChanges } = await pronghornApi.rpc("get_staged_changes_with_token", {
          ...
          // Overlay staged changes — fetch actual content from blob for non-delete operations
          const stagedList = (stagedChanges || []) as any[];
          await Promise.all(
            stagedList.map(async (s: any) => {
              if (s.operation_type === "delete") {
                fileContentMap.delete(s.file_path);
              } else {
                const { data: staged } = await pronghornApi.rpc(
                  "get_staged_file_content_with_token",
                  { p_repo_id: primeRepo.id, p_file_path: s.file_path, p_token: shareToken },
                );
                if (staged?.content !== null && staged?.content !== undefined) {
                  fileContentMap.set(s.file_path, staged.content);
                }
              }
            }),
          );
      ...
      if (selectedDatabaseItems.size > 0) {
        ...
        const renderDbResult = await pronghornApi.rpc("get_databases_with_token", {
          p_project_id: projectId,
          p_token: shareToken
        });
        ...
        try {
          const externalDbResult = await pronghornApi.rpc("get_db_connections_with_token", {
            p_project_id: projectId,
            p_token: shareToken
          });
        ...
        for (const databaseId of databaseGroups.keys()) {
          ...
          if (isExternal) {
            ...
            const [savedQueriesResult, migrationsResult] = await Promise.all([
              (pronghornApi.rpc as any)("get_saved_queries_by_connection_with_token", { 
                p_connection_id: databaseId, 
                p_token: shareToken 
              }),
              (pronghornApi.rpc as any)("get_migrations_by_connection_with_token", { 
                p_connection_id: databaseId, 
                p_token: shareToken 
              })
            ]);
            ...
            const [savedQueriesResult, migrationsResult] = await Promise.all([
              pronghornApi.rpc("get_saved_queries_with_token", { p_database_id: databaseId, p_token: shareToken }),
              pronghornApi.rpc("get_migrations_with_token", { p_database_id: databaseId, p_token: shareToken })
            ]);
            savedQueriesMap.set(databaseId, savedQueriesResult.data || []);
            migrationsMap.set(databaseId, migrationsResult.data || []);
          }
        ...
        for (const [databaseId, items] of databaseGroups) {
          ...
          for (const item of items) {
            ...
            if (item.type === "table") {
              try {
                const structureResponse = await pronghornApi.functions.invoke("manage-database", {
                  body: {
                    ...getBaseBody(),
                ...
                if (includeSampleData) {
                  const sampleResponse = await pronghornApi.functions.invoke("manage-database", {
                    body: {
                      ...getBaseBody(),
            ...
            if (item.type === "view") {
              try {
                const viewResponse = await pronghornApi.functions.invoke("manage-database", {
                  body: {
                    ...getBaseBody(),
            ...
            if (item.type === "function") {
              try {
                const funcResponse = await pronghornApi.functions.invoke("manage-database", {
                  body: {
                    ...getBaseBody(),
            ...
            if (item.type === "trigger") {
              try {
                const triggerResponse = await pronghornApi.functions.invoke("manage-database", {
                  body: {
                    ...getBaseBody(),
            ...
            if (item.type === "index") {
              try {
                const indexResponse = await pronghornApi.functions.invoke("manage-database", {
                  body: {
                    ...getBaseBody(),
            ...
            if (item.type === "sequence") {
              try {
                const seqResponse = await pronghornApi.functions.invoke("manage-database", {
                  body: {
                    ...getBaseBody(),
            ...
            if (item.type === "type") {
              try {
                const typeResponse = await pronghornApi.functions.invoke("manage-database", {
                  body: {
                    ...getBaseBody(),
