...
import { Wand2, Loader2, Download, RefreshCw, Sparkles, Eraser, Palette, Layers, Type, Grid2x2, Grid3x3, Square, RectangleHorizontal, RectangleVertical, AlertTriangle, Edit3 } from "lucide-react";
...
// Available FLUX image models via Azure AI Foundry
const IMAGE_MODELS = [
  { id: "flux-2-pro", label: "FLUX.2 Pro (Recommended)", description: "High-quality image generation & editing" },
  { id: "flux-kontext-pro", label: "FLUX Kontext Pro", description: "Context-aware image generation" },
];
...
export function EnhanceImageDialog({
  ...
  const handleEnhance = async (overridePrompt?: string) => {
    ...
    try {
      ...
      const response = await invokeEdgeFunction("enhance-image", {
        images,
        prompt: finalPrompt,
        model: selectedModel,
      });
  ...
  const handleSaveImage = async () => {
    ...
    try {
      ...
      const response = await invokeEdgeFunction("upload-artifact-image", {
      ...
      if (outputMode === "replace" && selectedArtifacts.size === 1) {
        ...
        const { error } = await pronghornApi.rpc("update_artifact_with_token", {
          p_id: artifactId,
        ...
        // Add as new artifact
        const { error } = await pronghornApi.rpc("insert_artifact_with_token", {
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {isCreating 
              ...
              : "Select images and apply AI-powered enhancements using FLUX"}
          </DialogDescription>
        </DialogHeader>
