...
interface SuperadminCloudManagerProps {
  type: "services" | "databases";
}

export function SuperadminCloudManager({ type }: SuperadminCloudManagerProps) {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ open: boolean; resource: CloudResource | null; isBulk: boolean }>({
    open: false,
    resource: null,
    isBulk: false,
  });
  ...
  const fetchResources = async () => {
    setLoading(true);
    try {
      const { data: { session } } = await pronghornApi.auth.getSession();
      if (!session) {
        toast({ title: "Authentication required", variant: "destructive" });
        return;
      }

      const { data, error } = await pronghornApi.functions.invoke("superadmin-render-management", {
  ...
  const handleAction = async (
    ...
    try {
      ...
      const { error } = await pronghornApi.functions.invoke("superadmin-render-management", { body });
      if (error) throw error;
  ...
  const performDelete = async (resource: CloudResource) => {
    try {
      const body: any = {
  ...
  const getStatusBadge = (resource: CloudResource) => {
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base sm:text-lg">
          ...
          All Cloud {type === "services" ? "Services" : "Databases"}
