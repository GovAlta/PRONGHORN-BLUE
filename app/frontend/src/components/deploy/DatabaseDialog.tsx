...
interface DatabaseDialogProps {
  ...
  onSuccess: () => void;
  primeRepoName?: string;
}

const PROVIDERS = [
  { value: "azure_postgres", label: "Azure PostgreSQL Database", enabled: true },
];
...
export function DatabaseDialog({
  ...
  useEffect(() => {
    if (mode === "edit" && database) {
      ...
      setForm({
        name: database.name || "",
        provider: database.provider || "azure_postgres",
        plan: database.plan || "basic_256mb",
        region: database.region || "canadacentral",
  ...
  }, [mode, database, open, primeRepoName]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === "create") {
        const { error } = await pronghornApi.rpc("insert_database_with_token", {
        ...
        const { error } = await pronghornApi.rpc("update_database_with_token", {
        ...
        if (database.render_postgres_id && form.plan !== database.plan) {
          const { error: renderError } = await pronghornApi.functions.invoke("render-database", {
            body: {
              action: "update",
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder={form.autoGenerateUser ? "Auto-generated" : "my_db_user"}
                      disabled={form.autoGenerateUser || (mode === "edit" && !!database?.database_internal_name)}
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder={form.autoGenerateName ? "Auto-generated" : "my_database"}
                      disabled={form.autoGenerateName || (mode === "edit" && !!database?.database_internal_name)}
                    />
                  </div>

                  {mode === "edit" && database?.database_internal_name && (
                    <div className="rounded-md bg-muted p-3 mt-4">
                      <p className="text-sm text-muted-foreground">
                        <strong>Note:</strong> Region and PostgreSQL version are inherited from the shared Azure PostgreSQL server. Only the name and IP allow list can be updated.
                      </p>
                    </div>
                  )}
                </div>
