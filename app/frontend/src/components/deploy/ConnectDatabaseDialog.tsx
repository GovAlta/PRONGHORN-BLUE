...
export function ConnectDatabaseDialog({
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const { data, error } = await pronghornApi.functions.invoke("manage-database", {
        body: {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing) {
        ...
        const { error } = await pronghornApi.rpc("update_db_connection_with_token", {
        ...
        const { data: newConnection, error } = await pronghornApi.rpc("insert_db_connection_with_token", {
        ...
        const connectionId = typeof newConnection === "object" && newConnection !== null 
          ? (newConnection as any).id 
          : newConnection as unknown as string;
        if (!connectionId) {
