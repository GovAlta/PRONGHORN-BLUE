...
const DeploymentCard = ({ deployment, shareToken, onUpdate, onSelect, isSelected }: DeploymentCardProps) => {
  ...
  const isTransitionalStatus = deployment.status === "pending" || deployment.status === "building" || deployment.status === "deploying";

  // Auto-refresh for transitional statuses
  const syncStatus = useCallback(async () => {
    if (!(deployment as any).azure_container_app_name) return;
    
    try {
      const { data, error } = await pronghornApi.functions.invoke("cloud-deployment", {
        body: {
          action: "status",
  ...
  }, [deployment.id, (deployment as any).azure_container_app_name, deployment.status, shareToken, onUpdate]);

  // Setup auto-refresh when status is transitional
  useEffect(() => {
    if (isTransitionalStatus && (deployment as any).azure_container_app_name) {
  ...
  }, [isTransitionalStatus, (deployment as any).azure_container_app_name, syncStatus]);

  const invokeCloudService = async (action: string) => {
  ...
  const handleCreate = () => invokeCloudService("create");
  const handleDeploy = () => invokeCloudService("deploy");
  const handleStart = () => invokeCloudService("start");
  const handleStop = () => invokeCloudService("stop");
  const handleRestart = () => invokeCloudService("restart");
  const handleSyncStatus = () => invokeCloudService("status");

  const handleDownloadPackage = async (mode: "full" | "env-only" = "full") => {
    setIsActionLoading(mode === "full" ? "download-full" : "download-env");
    try {
      const { data, error } = await pronghornApi.functions.invoke("generate-local-package", {
        body: {
          deploymentId: deployment.id,
          shareToken: shareToken || null,
          mode,
  ...
  // The create action reserves `azure_container_app_name` BEFORE dispatching
  // the workflow (FR-002). If dispatch fails, the name stays set but no Azure
  // resource ever existed — treat that case as "no cloud service" so the UI
  // still offers Create (and delete skips the cloud step).
  const hasReservedName = !!(deployment as any).azure_container_app_name;
  const createEverSucceeded =
    !!deployment.last_deployed_at || !!deployment.url;
  const hasCloudService =
    hasReservedName && (deployment.status !== "failed" || createEverSucceeded);

  const handleDelete = async () => {
    if (!confirm("Are you sure you want to delete this deployment? This will also delete the cloud service.")) return;
    
    setIsDeleting(true);
    try {
      // Only call cloud-delete when an Azure resource actually exists. A
      // failed create that never deployed has a reserved name but no resource.
      if (hasCloudService) {
        const { data, error } = await pronghornApi.functions.invoke("cloud-deployment", {
          body: {
            action: "delete",
        ...
        if (error) throw error;
        if (!data?.success) throw new Error(data?.error || "Failed to delete cloud service");
      }
      
      // Then delete from our database
      const { error } = await pronghornApi.rpc("delete_deployment_with_token", {
  ...
  return (
    <>
      ...
      <DeploymentLogsDialog
        ...
        deploymentId={deployment.id}
        shareToken={shareToken}
        cloudServiceId={(deployment as any).azure_container_app_name}
      />
