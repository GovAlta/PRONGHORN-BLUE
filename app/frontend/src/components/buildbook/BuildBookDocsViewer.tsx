  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (standards.length > 0) {
        ...
        const { data: selectedStandards } = await pronghornApi
          .from("standards")
          .select("*")
          .in("id", standardIds);
        ...
        if (selectedStandards && selectedStandards.length > 0) {
          ...
          // Load categories
          const { data: categories } = await pronghornApi
          ...
          for (const category of categories || []) {
            const { data: categoryStandards } = await pronghornApi
              .from("standards")
              .select("*")
      ...
      if (techStacks.length > 0) {
        ...
        const { data: allTechStacks } = await pronghornApi
          .from("tech_stacks")
          .select("*")
          .order("order_index");
