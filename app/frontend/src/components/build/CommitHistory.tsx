...
export function CommitHistory({ projectId, shareToken }: CommitHistoryProps) {
  ...
  const loadCommitHistory = async () => {
    ...
    try {
      ...
      // Load commit history
      const { data, error } = await pronghornApi.rpc("get_commit_history_with_token", {
  ...
  const handleRestore = async (commitId: string, commitSha: string) => {
    ...
    try {
      ...
      const { error: resetError } = await pronghornApi.rpc("reset_repo_files_with_token", {
        p_repo_id: repoId,
        p_token: shareToken || null,
      });
      ...
      const { data, error: pullError } = await pronghornApi.functions.invoke("sync-repo-pull", {
        body: {
          repoId: repoId,
          projectId: projectId,
