...
// Parse the URL and match against the registry host (suffix match) rather than
// a naive substring check, so URLs like "https://evil.com/npmjs.com" cannot be
// misclassified as a trusted registry.
const getLibraryType = (url: string): string | null => {
  ...
  const hostMatches = (...domains: string[]): boolean =>
    domains.some(
      (domain) => hostname === domain || hostname.endsWith(`.${domain}`),
    );

  if (hostMatches("npmjs.com", "npmjs.org")) return "npm";
  if (hostMatches("maven.org", "mvnrepository.com", "maven.apache.org"))
    return "maven";
  if (hostMatches("pypi.org", "anaconda.org", "anaconda.com")) return "python";
  if (hostMatches("nuget.org")) return "nuget";
  if (hostMatches("crates.io")) return "cargo";
  if (hostMatches("packagist.org")) return "composer";
  return null;
};
...
export function ResourceManager({ entityType, entityId, onResourcesChange }: ResourceManagerProps) {
  ...
  const loadResources = async () => {
    ...
    try {
      if (entityType === "tech_stack") {
        const { data, error } = await pronghornApi
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      const { error: uploadError } = await pronghornApi.storage
        .from("standard-attachments")
        .upload(fileName, file);

      if (uploadError) throw uploadError;

      const { data: { publicUrl } } = pronghornApi.storage
        .from("standard-attachments")
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingResource) {
        if (entityType === "tech_stack") {
          const { error } = await pronghornApi.from("tech_stack_resources").update({ ...baseData, tech_stack_id: entityId } as any).eq("id", editingResource.id);
          if (error) throw error;
        } else {
          const data = entityType === "standard_category" 
            ? { ...baseData, standard_category_id: entityId } 
            : { ...baseData, standard_id: entityId };
          const { error } = await pronghornApi.from("standard_resources").update(data as any).eq("id", editingResource.id);
          if (error) throw error;
        }
        toast.success("Resource updated");
      } else {
        if (entityType === "tech_stack") {
          const { error } = await pronghornApi.from("tech_stack_resources").insert({ ...baseData, tech_stack_id: entityId } as any);
          if (error) throw error;
        } else {
          const data = entityType === "standard_category" 
            ? { ...baseData, standard_category_id: entityId } 
            : { ...baseData, standard_id: entityId };
          const { error } = await pronghornApi.from("standard_resources").insert(data as any);
          if (error) throw error;
        }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      if (entityType === "tech_stack") {
        const { error } = await pronghornApi.from("tech_stack_resources").delete().eq("id", id);
        if (error) throw error;
      } else {
        const { error } = await pronghornApi.from("standard_resources").delete().eq("id", id);
        if (error) throw error;
      }
