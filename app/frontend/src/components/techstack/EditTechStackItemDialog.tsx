...
export function EditTechStackItemDialog({ open, onClose, itemId, parentId, onRefresh }: EditTechStackItemDialogProps) {
  ...
  const loadItem = async () => {
    if (!itemId) return;
    try {
      const { data, error } = await pronghornApi.from("tech_stacks").select("*").eq("id", itemId).single();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (itemId) {
        ...
        const { error } = await pronghornApi.from("tech_stacks").update(basePayload).eq("id", itemId);
        if (error) throw error;
        ...
        const { error } = await pronghornApi.from("tech_stacks").insert(insertPayload);
        if (error) throw error;
