...
export function TechStackSelector({ projectId, open, onClose, shareToken }: TechStackSelectorProps) {
  ...
  const loadStacks = async () => {
    const { data } = await pronghornApi.from("tech_stacks").select("*").order("name");
    setStacks(data || []);
  };

  const loadSelected = async () => {
    const { data } = await pronghornApi.rpc("get_project_tech_stacks_with_token", {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data: existing } = await pronghornApi.rpc("get_project_tech_stacks_with_token", {
        p_project_id: projectId,
        p_token: shareToken || null
      });
      ...
      if (existing && existing.length > 0) {
        for (const item of existing) {
          await pronghornApi.rpc("delete_project_tech_stack_with_token", {
            p_id: item.id,
            p_token: shareToken || null
          });
        }
      ...
      for (const techStackId of selectedIds) {
        await pronghornApi.rpc("insert_project_tech_stack_with_token", {
          p_project_id: projectId,
          p_token: shareToken || null,
  ...
  const linkStandardsFromStacks = async () => {
    try {
      ...
      const { data: stackStandards } = await pronghornApi.from("tech_stack_standards").select("standard_id, tech_stack_id").in("tech_stack_id", selectedIds);
      ...
      for (const req of requirements) {
        for (const ss of stackStandards) {
          try {
            await pronghornApi.rpc("insert_requirement_standard_with_token", {
