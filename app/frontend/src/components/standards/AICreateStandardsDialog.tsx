  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (categoryMode === "new") {
        ...
        const { data: newCategory, error: categoryError } = await pronghornApi
      ...
      if (files.length > 0) {
        ...
        for (const file of files) {
          ...
          const { data: uploadData, error: uploadError } = await pronghornApi.storage
          ...
          // Get public URL
          const { data: urlData } = pronghornApi.storage
            .from("requirement-sources")
      ...
      const { data, error } = await pronghornApi.functions.invoke("ai-create-standards", {
        body: {
          input: combinedInput,
