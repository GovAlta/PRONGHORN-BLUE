...
export function EditStandardDialog({ open, onClose, standardId, parentId, categoryId, onRefresh }: EditStandardDialogProps) {
  ...
  const loadStandard = async () => {
    ...
    try {
      const { data, error } = await pronghornApi.from("standards").select("*").eq("id", standardId).single();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (standardId) {
        const { error } = await pronghornApi.from("standards").update(payload).eq("id", standardId);
        if (error) throw error;
        toast.success("Standard updated");
      } else {
        const { error } = await pronghornApi.from("standards").insert(payload);
        if (error) throw error;
