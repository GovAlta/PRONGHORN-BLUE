...
function isRasterImage(filename: string): boolean {
  const ext = filename.toLowerCase().split(".").pop();
  return ["png", "jpg", "jpeg", "gif", "bmp", "webp", "tiff", "tif"].includes(
    ext || "",
  );
}
...
function htmlToMarkdown(html: string): string {
  ...
  // Remove remaining HTML tags (robust against nested/revealed tags)
  md = stripHtmlTags(md);

  // Decode HTML entities. Decode &amp; LAST so that an already-encoded entity
  // such as "&amp;lt;" is not double-unescaped into "<".
  md = md.replace(/&nbsp;/g, " ");
  md = md.replace(/&lt;/g, "<");
  md = md.replace(/&gt;/g, ">");
  md = md.replace(/&quot;/g, '"');
...
/**
 * Remove HTML tags from a string. Applies the replacement until the result is
 * stable, so that tags revealed by removing an outer match (e.g. "<<b>>") are
 * also stripped — a single regex pass is an incomplete multi-character
 * sanitizer.
 */
...
export async function rasterizeDocx(
  ...
  // Sanitize the docx-derived HTML to strip scripts/event handlers before it is
  // injected via innerHTML (mammoth output is user-document controlled).
  ...
  const cancelBtn = progressModal.querySelector(
    "#docx-raster-cancel",
  ...
  const updateProgressText = (text: string) => {
    const progressText = progressModal.querySelector(
      "#docx-raster-progress-text",
    );
    if (progressText) {
  ...
  try {
    ...
    const elements = contentWrapper.querySelectorAll(
      "p, h1, h2, h3, h4, h5, h6, li, tr, div, table, ul, ol, blockquote",
    );
    ...
    while (currentBreakTarget < contentHeight) {
      ...
      for (const bounds of elementBounds) {
        ...
        if (
          bounds.bottom <= currentBreakTarget &&
          bounds.bottom > currentBreakTarget - 150
        ) {
