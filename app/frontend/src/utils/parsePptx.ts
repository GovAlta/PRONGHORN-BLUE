...
async function parseThemeColors(zip: JSZip): Promise<Record<string, string>> {
  ...
  try {
    ...
    console.log(
      `[PPTX Parser] Found ${themeFiles.length} theme files:`,
      themeFiles,
    );
 ...
 */
async function parseColorMapFromMasters(
  zip: JSZip,
  ...
  try {
    ...
    console.log(
      `[PPTX Parser] Found ${masterFiles.length} slide master files:`,
      masterFiles,
    );
...
const placeholderPositions: Map<string, PlaceholderPosition> = new Map();
...
async function parsePlaceholderPositions(zip: JSZip): Promise<void> {
  ...
  console.log(
    `[PPTX Parser] Parsing placeholder positions from ${masterFiles.length} slide masters`,
  );
    ...
    try {
      ...
      for (let i = 0; i < spElements.length; i++) {
        ...
        if (transform) {
          ...
          console.log(
            `[PPTX Parser] Placeholder '${phType}' position: x=${transform.x}, y=${transform.y}, w=${transform.width}, h=${transform.height}`,
          );
        }
      }
    } catch (error) {
      console.warn(
        `Failed to parse placeholder positions from ${masterPath}:`,
        error,
      );
    }
  }
  ...
  console.log(
    `[PPTX Parser] Parsing placeholder positions from ${layoutFiles.length} slide layouts`,
  );
  ...
  for (const layoutPath of layoutFiles) {
    ...
    try {
      ...
      console.warn(
        `Failed to parse placeholder positions from ${layoutPath}:`,
        error,
      );
    }
  }
  ...
  console.log(
    "[PPTX Parser] Total placeholder positions cached:",
    Object.fromEntries(placeholderPositions),
  );
}
...
async function parseLayoutPlaceholderStyles(
  ...
  try {
    ...
    for (let i = 0; i < spElements.length; i++) {
        ...
        if (lstStyle) {
          ...
          if (lvl1pPr) {
            ...
            if (defRPr) {
              ...
              const solidFill = defRPr.getElementsByTagNameNS(
                NS.a,
                "solidFill",
              )[0];
      ...
      console.log(
        `[PPTX Parser] Layout placeholder '${phType}': fontColor=${fontColor}, fontSize=${fontSize}, fontFamily=${fontFamily}, pos=(${transform?.x}, ${transform?.y})`,
      );
    }
  } catch (error) {
    console.warn(
      `Failed to parse layout placeholder styles from ${layoutPath}:`,
      error,
    );
  }
...
function extractParagraphsFromShape(sp: Element): PptxParagraph[] {
  ...
  const txBody =
    sp.getElementsByTagNameNS(NS.p, "txBody")[0] ||
...
async function extractSlideBackground(
  ...
  if (slideFile) {
    ...
    if (cSld) {
      ...
      if (bg) {
        ...
        if (color) {
          console.log(
            `[PPTX Parser] Slide ${slideIndex + 1} background from slide: ${color}`,
          );
          return color;
        }
      }
  ...
  for (const relId in slideRels) {
    ...
    if (target.includes("slideLayouts")) {
      // Normalize path (strip ALL leading "../" traversal segments)
    ...
    if (layoutFile) {
      ...
      if (cSld) {
        ...
        if (bg) {
          ...
          if (color) {
            console.log(
              `[PPTX Parser] Slide ${slideIndex + 1} background from layout: ${color}`,
            );
            return color;
          }
        }
      ...
      if (masterFile) {
        ...
        if (cSld) {
          ...
          if (bg) {
            ...
            if (color) {
              console.log(
                `[PPTX Parser] Slide ${slideIndex + 1} background from master: ${color}`,
              );
              return color;
            }
          }
  ...
  // No background found
  console.log(
    `[PPTX Parser] Slide ${slideIndex + 1} no background color found`,
  );
  return undefined;
}
...
function extractShapesFromXml(
  ...
  for (let i = 0; i < spElements.length; i++) {
    ...
    if (transform) {
      ...
      console.log(
        `[PPTX Parser] Shape ${i}: Using layout position for placeholder '${placeholderType}': x=${x}, y=${y}`,
      );
      ...
      console.log(
        `[PPTX Parser] Shape ${i}: Using master position for placeholder '${placeholderType}': x=${x}, y=${y}`,
      );
    } else {
    ...
    console.log(
      `[PPTX Parser] Shape ${i}: x=${x}, y=${y}, w=${width}, h=${height}, paragraphs=${paragraphs.length}, placeholder=${placeholderType || "none"}, hasTransform=${!!transform}`,
    );
    ...
    if (layoutStyle) {
      ...
      if (!fontColor && layoutStyle.fontColor) {
        ...
        console.log(
          `[PPTX Parser] Shape ${i} ('${placeholderType}'): Inheriting fontColor=${fontColor} from layout`,
        );
      }
      ...
      if (!fontSize && layoutStyle.fontSize) {
        ...
        console.log(
          `[PPTX Parser] Shape ${i} ('${placeholderType}'): Inheriting fontSize=${fontSize} from layout`,
        );
      }
...
async function parseSlide(
  ...
  if (layoutPath) {
    ...
    console.log(
      `[PPTX Parser] Slide ${slideIndex + 1} uses layout ${layoutPath} with ${layoutStyles.size} placeholder styles`,
    );
  }
