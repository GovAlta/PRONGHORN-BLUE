...
export async function parseExcelFile(file: File): Promise<ExcelData> {
  ...
  workbook.eachSheet((worksheet) => {
    ...
    try {
        ...
        try {
          row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
            ...
            try {
                ...
                if (typeof cell.value === "object") {
                  ...
                  // Formula cell - get the calculated result, NOT the formula itself
                    ...
                    if (
                      ...
                      if (
                        typeof cellValue.result === "object" &&
                        cellValue.result !== null
                      ) {
                  ...
                  // Unknown object - avoid [object Object]
                    ...
                    if (
                      ...
                      "valueOf" in cellValue &&
                      typeof cellValue.valueOf === "function"
                    ) {
                      ...
                      if (typeof v !== "object") {
                        ...
                        console.warn(
                          `Unhandled cell object at row ${rowNumber}, col ${colNumber}:`,
                      ...
                      console.warn(
                        `Unknown cell object at row ${rowNumber}, col ${colNumber}:`,
                        Object.keys(cellValue),
                      );
                      value = "";
                    }
              ...
              console.warn(
                `Cell parsing error at row ${rowNumber}, col ${colNumber}:`,
                cellError,
              );
              value = "";
            }
          ...
          if (
            rowCellError.message?.includes("filterButton") ||
            rowCellError.message?.includes("Cannot set properties of undefined")
          ) {
            warnings.push(
              `Row ${rowNumber}: Some cell formatting was skipped due to Excel filter features`,
            );
          } else {
            console.warn(
              `Row ${rowNumber} cell iteration error:`,
              rowCellError,
            );
          }
        }
      ...
      warnings.push(
        `Sheet "${worksheet.name}": Some data may be incomplete due to parsing issues`,
      );
    }
  ...
  if (sheets.length === 0) {
    throw new Error(
      "No data could be extracted from the Excel file. The file may be corrupted or in an unsupported format.",
    );
  }
...
export function formatExcelDataAsJson(
  ...
  sheets.forEach((sheet) => {
    ...
    const headers =
      sheet.headers.length > 0 ? sheet.headers : sheet.rows[0] || [];
    const dataRows: any[] = [];
