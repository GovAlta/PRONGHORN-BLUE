...
export default function Standards() {
  ...
  const loadCategories = async () => {
    const { data } = await pronghornApi.rpc("get_standard_categories");
    setCategories(data || []);
  };

  const loadStandards = async () => {
    const { data } = await pronghornApi.rpc("get_standards_with_attachments");
    if (data) {
  ...
  const handleAddCategory = async () => {
    ...
    const { data: { user } } = await pronghornApi.auth.getUser();
    const { data: profile } = await pronghornApi.from("profiles").select("org_id").eq("user_id", user?.id).single();
    
    const { error } = await pronghornApi.from("standard_categories").insert({
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    ...
    const { error } = await pronghornApi.from("standard_categories").delete().eq("id", categoryId);
  ...
  const handleUpdateCategory = async (categoryId: string, name: string, description: string, longDescription?: string) => {
    ...
    const { error } = await pronghornApi
