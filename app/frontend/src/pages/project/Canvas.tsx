...
import { Camera, Lasso as LassoIcon, Image, ChevronRight, Wrench, Sparkles, FileSearch, AlignLeft, AlignVerticalJustifyStart, AlignHorizontalDistributeCenter, AlignVerticalDistributeCenter, Grid3x3, ImagePlus, Eye, Trash2 } from "lucide-react";
...
function CanvasFlow() {
  ...
  const visibleNodes = useMemo(() => {
    ...
    // When visibleNodeTypes hasn't loaded yet (empty set), show ALL nodes
    // so the canvas isn't blank while node types are loading from the API
    // Also show nodes with types not in the known set (e.g. AI-generated custom types)
    const knownTypes = allNodeTypes ? new Set(allNodeTypes.map(nt => nt.system_name)) : new Set<string>();
    const typeFiltered = visibleNodeTypes.size === 0
      ? nodes
      : nodes.filter((node) => {
          const nodeType = node?.data?.type;
          if (!nodeType) return true; // No type = show
          if (!knownTypes.has(nodeType)) return true; // Unknown type (AI-generated) = show
          return visibleNodeTypes.has(nodeType); // Known type = check toggle
        });
  ...
  }, [nodes, visibleNodeTypes, allNodeTypes, layers, isIsolateActive, edges]);
  ...
  const handleEdgeDelete = useCallback(
    async (edgeId: string) => {
      ...
      try {
        await apiClient.delete(`/api/v1/canvas/${projectId}/edges/${edgeId}`);
      } catch (error) {
  ...
  const handleNodeDelete = useCallback(
    async (nodeId: string) => {
      ...
      // Delete from database using APIM backend
      try {
        await apiClient.delete(`/api/v1/canvas/${projectId}/nodes/${nodeId}`);
        toast({
          title: "Node deleted",
        });
      } catch (error) {
    ...
    async (generatedNodes: any[], generatedEdges: any[]) => {
      try {
        ...
        for (const genEdge of generatedEdges) {
          ...
          const sourceId = nodeIdMap.get(genEdge.source) // By label
  ...
  const handleClearCanvas = useCallback(async () => {
    ...
    // Delete all nodes from database using APIM backend
    for (const node of nodes) {
      try {
        await apiClient.delete(`/api/v1/canvas/${projectId}/nodes/${node.id}`);
      } catch (error) {
