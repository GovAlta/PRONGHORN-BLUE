...
export default function Standards() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { data: techStacksData } = await pronghornApi
        .from("tech_stacks")
        .select("*")
        .is("parent_id", null)
        .is("type", null)
      ...
      const { data: projectStandards } = await pronghornApi.rpc("get_project_standards_with_token", {
        p_project_id: projectId,
        p_token: shareToken || null
      });

      const { data: projectTechStacks } = await pronghornApi.rpc("get_project_tech_stacks_with_token", {
        p_project_id: projectId,
        p_token: shareToken || null
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const existing of standardsToRemove) {
        await pronghornApi.rpc("delete_project_standard_with_token", {
          p_id: existing.id,
          p_token: shareToken || null
        });
      }
      ...
      for (const standardId of standardsToAdd) {
        await pronghornApi.rpc("insert_project_standard_with_token", {
          p_project_id: projectId!,
          p_token: shareToken || null,
