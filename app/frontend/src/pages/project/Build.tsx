...
import { stagingChannel, repoFilesChannel } from "@/lib/repoChannels";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function Build() {
  ...
  const loadFiles = useCallback(async () => {
    ...
    try {
      ...
      const { data: committedFiles, error: filesError } = await pronghornApi.rpc(
      ...
      const { data: staged, error: stagedError } = await pronghornApi.rpc(
      ...
      const stagedMap = new Map<string, any>((staged || []).map((s: any) => [s.file_path, s]));
      ...
      // Add staged files whose paths don't exist in committed repo_files.
      // This covers 'add', 'create', 'rename', and any re-staged files whose
      // operation_type may have been updated (e.g. 'modify' on a new file).
  ...
  const refreshFilesAndStagingPanel = useCallback(() => {
    loadFiles();
    setStagingRefreshTrigger(prev => prev + 1);
  }, [loadFiles]);
  ...
  useEffect(() => {
    ...
    // Channel for repo_files_refresh broadcasts
    const filesChannel = pronghornApi
      .channel(repoFilesChannel(projectId))
      .on(
        "broadcast",
    ...
    const stagingSubscription = pronghornApi
      .channel(stagingChannel(defaultRepo.id))
      .on(
        "broadcast",
        ...
        (payload) => {
          ...
          // Evict clean staged buffer entries so next open fetches fresh content
          clearCleanStagedEntriesRef.current?.(currentPathRef.current);
    ...
    return () => {
      pronghornApi.removeChannel(filesChannel);
      pronghornApi.removeChannel(stagingSubscription);
    };
  ...
  const clearCleanStagedEntriesRef = useRef(clearCleanStagedEntries);
  ...
  clearCleanStagedEntriesRef.current = clearCleanStagedEntries;
  ...
  const handleRenameFile = (fileId: string, path: string) => {
    const isFolder = !files.some((file) => file.path === path);
  ...
  const handleConfirmRename = async (newName: string) => {
    ...
    const existingPaths = files.map(f => f.path);
    const folderChildren = itemToRename.type === "folder"
      ? files.filter((file) => file.path.startsWith(`${oldPath}/`))
      : [];
    const excludedPaths = itemToRename.type === "folder"
      ? new Set(folderChildren.map((file) => file.path))
      : new Set([oldPath]);
    newPath = getUniqueFilePath(newPath, existingPaths.filter((path) => !excludedPaths.has(path)));
    ...
    try {
      if (itemToRename.type === "folder") {
        if (folderChildren.length === 0) {
          toast.error("Folder has no files to rename");
          return;
        }

        await Promise.all(folderChildren.map((file) => {
        ...
        toast.success(`Renamed folder to ${newPath.split("/").pop()}`);
      ...
      if (stagedFile?.new_content) {
        ...
        if (fileRecord) {
          const { data: fileData } = await pronghornApi.rpc("get_file_content_with_token", {
  ...
  const handleDeleteFile = async () => {
    ...
    try {
      ...
      if (!currentFile.isStaged) {
        ...
        const { data: fileData, error: readError } = await pronghornApi.rpc("get_file_content_with_token", {
