...
export default function Repository() {
  ...
  const loadFileStructure = async () => {
    ...
    try {
      // Fetch committed structure and staged metadata in parallel
      const [committedResult, stagedResult] = await Promise.all([
        pronghornApi.rpc("get_file_structure_with_token", {
          p_repo_id: selectedRepoId,
          p_token: shareToken || null,
        }),
        pronghornApi.rpc("get_staged_changes_metadata_with_token", {
          p_repo_id: selectedRepoId,
          p_token: shareToken || null,
        }),
      ]);

      if (committedResult.error) throw committedResult.error;

      const committedFiles = (committedResult.data as any[]) || [];
      const stagedChanges = (stagedResult.data as any[]) || [];
      setStagedChanges(stagedChanges);

      // Build a set of paths that are being renamed away so they can be hidden
      ...
      // Start from committed files, excluding those replaced by renames and deleted
      const deletedPaths = new Set(
        stagedChanges
          .filter((s: any) => s.operation_type === "delete")
          .map((s: any) => s.file_path)
      );
      const filteredCommitted = committedFiles.filter(
        (f: any) => !renamedFromPaths.has(f.path) && !deletedPaths.has(f.path)
      );

      // Collect staged-only paths (add / create / rename destination)
      const committedPaths = new Set(filteredCommitted.map((f: any) => f.path));
      const stagedOnlyFiles = stagedChanges
        .filter(
          (s: any) =>
            (s.operation_type === "add" ||
              s.operation_type === "create" ||
              s.operation_type === "rename") &&
            !committedPaths.has(s.file_path)
        )
        .map((s: any) => ({ path: s.file_path, type: "file", id: s.id }));
      ...
      // Rebuild the path → { id, isStaged } map for handleFileSelect lookups
      const newFileIdMap = new Map<string, { id: string | null; isStaged: boolean }>();
      for (const f of filteredCommitted) {
        newFileIdMap.set(f.path, { id: f.id ?? null, isStaged: false });
      }
      for (const f of stagedOnlyFiles) {
        // Staged-only files: id is the staging row id — do NOT use it as fileId
        // (CodeEditor treats fileId as a repo_files id; staged-only files have none)
        newFileIdMap.set(f.path, { id: null, isStaged: true });
      }
      fileIdMapRef.current = newFileIdMap;

      const tree = buildFileTree(allFiles);
      setFileStructure(tree);

      // Load committed file contents for content search (staged-only files have no committed content)
      const filePaths = filteredCommitted
        .filter((f: any) => f.type === "file")
        .map((f: any) => f.path);
      
      if (filePaths.length > 0) {
        const { data: filesData, error: filesError } = await pronghornApi.rpc("get_repo_files_with_token", {
          p_repo_id: selectedRepoId,
          p_token: shareToken || null,
  ...
  const handleFileSelect = (path: string) => {
    // Resolve id and staged flag from the local map populated by loadFileStructure —
    // no extra RPC needed, and fileId is never a staging row id.
    const entry = fileIdMapRef.current.get(path);
    const fileId = entry?.id ?? null;
    const isStaged = entry?.isStaged ?? false;

    // Set all three together so the CodeEditor key (fileId ?? filePath) remounts once
  ...
  const handleDeleteRepo = async (repoId: string) => {
    ...
    try {
      const { error } = await pronghornApi.rpc("delete_project_repo_with_token", {
        p_repo_id: repoId,
        p_token: shareToken || null,
      });
  ...
  const handleCreateEmpty = async (name: string, isPrivate: boolean) => {
    ...
    try {
      const { error } = await pronghornApi.functions.invoke("create-empty-repo", {
        body: {
          projectId,
          repoName: name,
      ...
      toast.success("Empty repository created successfully");
      refetch();
    } catch (error) {
      console.error("Error creating empty repository:", error);
      toast.error("Failed to create empty repository");
    }
  };
  ...
  const handleCreateFromTemplate = async (name: string, templateOrg: string, templateRepo: string, isPrivate: boolean) => {
    ...
    try {
      const { error } = await pronghornApi.functions.invoke("create-repo-from-template", {
        body: {
          projectId,
          repoName: name,
          templateOrg,
      ...
      toast.success("Repository created from template successfully");
      refetch();
    } catch (error) {
  ...
  const handleClonePublic = async (name: string, sourceOrg: string, sourceRepo: string, sourceBranch: string, isPrivate: boolean) => {
    ...
    try {
      const { error } = await pronghornApi.functions.invoke("clone-public-repo", {
        body: {
          projectId,
          repoName: name,
          sourceOrg,
      ...
      toast.success("Public repository cloned successfully");
      refetch();
    } catch (error) {
      console.error("Error cloning public repository:", error);
      toast.error("Failed to clone public repository");
    }
  };
  ...
  const handleLinkExisting = async (org: string, repo: string, branch: string, pat?: string) => {
    ...
    try {
      const { error } = await pronghornApi.functions.invoke("link-existing-repo", {
        body: {
          projectId,
          organization: org,
          repo,
  ...
  const handleSyncWithConfig = async (config: SyncConfig, isPush: boolean) => {
    try {
      ...
      if (isPush) {
        ...
        if (!primeRepo) {
          toast.error("Please set a Prime repository first");
          return;
        }

        if (stagedChanges.length > 0) {
          toast.error("Uncommitted changes will not be pushed", {
            description: `${stagedChanges.length} staged file(s) have not been committed and will be missing from the push. Commit them first, then push.`,
          });
          return;
        }
        ...
        // Push to Prime first
        const { error: primeError } = await pronghornApi.functions.invoke("sync-repo-push", {
          body: {
            repoId: primeRepo.id,
            projectId: projectId,
        ...
        if (primeError) {
          ...
          toast.error("Push failed", {
            description: primeError.message || "Failed to push to Prime repository",
          });
          return;
        }
        ...
        const mirrorPromises = mirrorRepos.map(async (repo) => {
          try {
            const { error } = await pronghornApi.functions.invoke("sync-repo-push", {
              body: {
                repoId: repo.id,
        ...
        toast(failed === 0 ? "Push complete" : "Push completed with errors", {
        ...
        try {
          const { error } = await pronghornApi.functions.invoke("sync-repo-pull", {
            body: {
              repoId: primeRepo.id,
              projectId: projectId,
              shareToken: shareToken,
          ...
          toast.success(`Pulled from Prime: ${primeRepo.organization}/${primeRepo.repo}`);

          loadFileStructure();
        } catch (err: any) {
          setSyncStatus({ [primeRepo.id]: "error" });
          toast.error(err.message || "Failed to pull from Prime repository");
        }
      }
      ...
      toast.error(error.message || `Failed to ${isPush ? "push to" : "pull from"} repositories`);
      // Keep error status visible for 5 seconds so user can see which repos failed
      setTimeout(() => {
        setSyncStatus({});
      }, 5000);
    }
  ...
  const handleFileCreate = async (path: string, isFolder: boolean) => {
    ...
    try {
      if (isFolder) {
        // Stage a .gitkeep placeholder so the folder appears in the tree
        await pronghornApi.functions.invoke("staging-operations", {
          body: {
            action: "stage",
            repoId: selectedRepoId,
            shareToken: shareToken || null,
            filePath: `${path}/.gitkeep`,
            operationType: "add",
            newContent: "",
          },
        });
  ...
  const handleFileRename = async (oldPath: string, newPath: string) => {
    ...
    try {
      ...
      const { data, error } = await pronghornApi.rpc("get_file_structure_with_token", {
        p_repo_id: selectedRepoId,
        p_token: shareToken || null,
      });
      ...
      if (file) {
        ...
        if (isFolder) {
          ...
          await pronghornApi.rpc("rename_folder_with_token", {
            p_repo_id: selectedRepoId,
          ...
          // Rename single file
          await pronghornApi.rpc("rename_file_with_token", {
            p_file_id: file.id,
            p_new_path: newPath,
            p_token: shareToken || null,
          });
  ...
  const handleFileDelete = async (path: string) => {
    ...
    try {
      ...
      for (const file of filesToDelete) {
        await pronghornApi.rpc("delete_file_with_token", {
          p_file_id: file.id,
          p_token: shareToken || null,
        });
      }
  ...
  const handleZipUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (fileEntries.length === 0) {
        toast.error("No files found in the ZIP archive");
                  ...
                  <Card className="p-0 overflow-hidden bg-[#1e1e1e] border-none">
                    ...
                    <ResizablePanelGroup direction="horizontal" className="min-h-[700px] bg-[#1e1e1e]">
                      ...
                      <ResizablePanel defaultSize={75}>
                        <CodeEditor
                          key={selectedFileId ?? selectedFilePath ?? ""}
