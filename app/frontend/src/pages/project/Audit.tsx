...
import { useAuditPipeline } from "@/hooks/useAuditPipeline";
...
import { PlayCircle, Pause, StopCircle, Settings2, Activity, Grid3X3, CircleDot, RefreshCw, Network, Brain, RotateCcw, Loader2, Save, Download } from "lucide-react";
...
export default function Audit() {
  ...
  // New pipeline hook
  const { runPipeline, isRunning: isPipelineRunning, progress: pipelineProgress, steps: pipelineSteps, error: pipelineError, results: pipelineResults, clearResults: clearPipelineResults, restartStep: restartPipelineStep, reconstructStepsFromActivity, stepMode, setStepMode, pausedAfterStep, continueToNextStep } = useAuditPipeline();
  ...
  useEffect(() => {
    ...
    const loadSessions = async () => {
      const { data, error } = await pronghornApi.rpc("get_audit_sessions_list_with_token", {
  ...
  const resumeOrchestrator = useCallback(async (sessionToResume: AuditSession) => {
    ...
    try {
      const { data, error: orchestratorError } = await pronghornApi.functions.invoke("audit-orchestrator", {
        body: {
