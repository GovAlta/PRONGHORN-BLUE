  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (standardIdsArray.length > 0) {
        const { error: standardsError } = await pronghornApi.from("build_book_standards").insert(
      ...
      if (techStackIdsArray.length > 0) {
        const { error: techStacksError } = await pronghornApi.from("build_book_tech_stacks").insert(
