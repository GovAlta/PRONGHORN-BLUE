...
export default function Dashboard() {
  ...
  const { data: projects = [], isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      if (!user?.id) return [];
      try {
        // Use APIM backend for projects
        const data = await apiClient.get<any[]>("/api/v1/projects");
        return (data || []).map((p: any) => ({
          ...
          budget: p.budget,
          scope: p.scope,
          splashImageUrl: p.splash_image_url,
        }));
      } catch (error) {
  ...
  const { data: linkedProjects = [], refetch: refetchLinked } = useQuery({
    ...
    queryFn: async () => {
      if (!user?.id) return [];
      try {
        const response = await apiClient.post<{ data: any[], error: any }>("/api/v1/rpc/get_linked_projects", {});
  ...
  const { data: publishedProjects = [], isLoading: isLoadingGallery } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await apiClient.post<{ data: any[], error: any }>("/api/v1/rpc/get_published_projects", {});
        return (response?.data || []) as PublishedProject[];
      } catch (error) {
  ...
  const getLinkedProjectToken = async (projectId: string): Promise<string | null> => {
    try {
      const data = await apiClient.get<any[]>(`/api/v1/profile-linked-projects?project_id=${projectId}`);
  ...
  const handleSaveProject = async (projectId: string, shareToken: string) => {
    ...
    try {
      const { error } = await pronghornApi.rpc("save_anonymous_project_to_user", {
