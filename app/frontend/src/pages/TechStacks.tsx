...
export default function TechStacks() {
  ...
  const loadTechStacks = async () => {
    ...
    const { data } = await pronghornApi.rpc("get_tech_stacks_root");
    setTechStacks(data || []);
  };
  ...
  const handleAddTechStack = async () => {
    ...
    const { error } = await pronghornApi.from("tech_stacks").insert({
  ...
  const handleDeleteTechStack = async (techStackId: string) => {
    ...
    const { error } = await pronghornApi.from("tech_stacks").delete().eq("id", techStackId);
            ...
            <Card>
              <CardContent className="pt-4 md:pt-6 p-4 md:p-6">
                <div className="flex flex-col md:flex-row items-stretch md:items-center gap-2">
                  ...
                  />
                  <Button onClick={handleAddTechStack} size="lg" className="flex-1 md:flex-none">
