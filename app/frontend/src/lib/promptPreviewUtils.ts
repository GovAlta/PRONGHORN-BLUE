...
export function generateToolsListText(manifest: ToolsManifest | null): string {
  ...
  const renderToolCategory = (
    title: string,
    toolsObj: Record<string, any> | undefined,
    description?: string,
  ) => {
    if (!toolsObj) return;
...
export function generateResponseSchemaText(
...
export function generatePromptPreview(
  ...
  for (const section of sortedSections) {
    ...
    for (const [variable, value] of Object.entries(variables)) {
      // Escape ALL regex metacharacters in the variable name (not just braces)
      // before building the dynamic RegExp, to avoid regex injection / broken
      // substitution when a name contains characters like ".", "*", or "(".
...
export function identifyRuntimePlaceholders(
  text: string,
): { start: number; end: number; placeholder: string }[] {
