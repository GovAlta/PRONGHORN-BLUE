...
export async function fetchProjectData(projectId: string, shareToken: string | null) {
  ...
  const { data: project } = await pronghornApi.rpc("get_project_with_token", {
    p_project_id: projectId,
    p_token: shareToken
  });

  // Fetch requirements via RPC
  const { data: requirements } = await pronghornApi.rpc("get_requirements_with_token", {
    p_project_id: projectId,
    p_token: shareToken
  });

  // Fetch canvas nodes via RPC
  const { data: canvasNodes } = await pronghornApi.rpc("get_canvas_nodes_with_token", {
    p_project_id: projectId,
    p_token: shareToken
  });

  // Fetch canvas edges via RPC
  const { data: canvasEdges } = await pronghornApi.rpc("get_canvas_edges_with_token", {
    p_project_id: projectId,
    p_token: shareToken
  });

  // Fetch project standards via RPC
  const { data: projectStandardsRaw } = await pronghornApi.rpc("get_project_standards_with_token", {
    p_project_id: projectId,
    p_token: shareToken
  });
  ...
  const projectStandards = await Promise.all(
    (projectStandardsRaw || []).map(async (ps: any) => {
      const { data: standard } = await pronghornApi
        .from("standards")
  ...
  const { data: projectTechStacksRaw } = await pronghornApi.rpc("get_project_tech_stacks_with_token", {
    p_project_id: projectId,
    p_token: shareToken
  });
  ...
  const projectTechStacks = await Promise.all(
    (projectTechStacksRaw || []).map(async (pts: any) => {
      const { data: techStack } = await pronghornApi
        .from("tech_stacks")
  ...
  // Fetch saved specification (shareToken is guaranteed by the guard above)
  const { data: spec } = await pronghornApi.rpc("get_project_specification_with_token", {
    p_project_id: projectId,
    p_token: shareToken
  });
  ...
  if (requirements) {
    for (const req of requirements) {
      const { data: files } = await pronghornApi.storage
  ...
  if (projectStandards) {
    for (const ps of projectStandards) {
      ...
      if (standard) {
        const { data: attachments } = await pronghornApi
  ...
  const { data: artifacts } = await pronghornApi.rpc("get_artifacts_with_token", {
    p_project_id: projectId,
    p_token: shareToken
  });

  // Fetch chat sessions via RPC
  const { data: chatSessions } = await pronghornApi.rpc("get_chat_sessions_with_token", {
    p_project_id: projectId,
    p_token: shareToken
  });
  ...
  if (chatSessions) {
    for (const session of chatSessions) {
      const { data: messages } = await pronghornApi.rpc("get_chat_messages_with_token", {
