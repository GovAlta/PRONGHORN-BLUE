...
export async function getDatabaseConnectionSecrets(
  ...
  const { data, error } = await pronghornApi.functions.invoke("database-connection-secrets", {
    body: {
      action: "get",
      connectionId,
      shareToken,
...
export async function setDatabaseConnectionString(
  ...
  const { error } = await pronghornApi.functions.invoke("database-connection-secrets", {
    body: {
      action: "set",
      connectionId,
      shareToken,
