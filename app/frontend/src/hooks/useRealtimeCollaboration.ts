...
import { Database } from "@/integrations/pronghorn-api/types";
...
export function useRealtimeCollaboration(
  ...
  const loadCollaboration = useCallback(async () => {
    ...
    try {
      const { data, error } = await pronghornApi.rpc("get_artifact_collaboration_with_token", {
  ...
  const loadMessages = useCallback(async () => {
    ...
    try {
      const { data, error } = await pronghornApi.rpc("get_collaboration_messages_with_token", {
        p_collaboration_id: collaborationId,
        p_token: shareToken,
      });
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const { data, error } = await pronghornApi.rpc("get_collaboration_history_with_token", {
        p_collaboration_id: collaborationId,
        p_token: shareToken,
      });
  ...
  const loadBlackboard = useCallback(async () => {
    ...
    try {
      const { data, error } = await pronghornApi.rpc("get_collaboration_blackboard_with_token", {
        p_collaboration_id: collaborationId,
        p_token: shareToken,
      });
  ...
  const sendMessage = useCallback(async (
    ...
    try {
      const { data, error } = await pronghornApi.rpc("insert_collaboration_message_with_token", {
        p_collaboration_id: collaborationId,
        p_token: shareToken || "",
        p_role: role,
        p_content: content,
  ...
  const insertEdit = useCallback(async (
    ...
    try {
      const { data, error } = await pronghornApi.rpc("insert_collaboration_edit_with_token", {
        p_collaboration_id: collaborationId,
        p_token: shareToken || "",
      ...
      await pronghornApi.rpc("update_artifact_collaboration_with_token", {
        p_collaboration_id: collaborationId,
        p_token: shareToken,
  ...
  const restoreToVersion = useCallback(async (versionNumber: number) => {
    ...
    try {
      const { data, error } = await pronghornApi.rpc("restore_collaboration_version_with_token", {
        p_collaboration_id: collaborationId,
        p_token: shareToken,
  ...
  const addBlackboardEntry = useCallback(async (
    ...
    try {
      const { data, error } = await pronghornApi.rpc("insert_collaboration_blackboard_with_token", {
        p_collaboration_id: collaborationId,
        p_token: shareToken || "",
  ...
  const updatePresence = useCallback((
    ...
    if (presenceChannelRef.current && typeof presenceChannelRef.current.send === "function") {
  ...
  useEffect(() => {
    ...
    // Main channel for data changes (broadcast-based, replaces postgres_changes)
    ...
    const presenceChannel = pronghornApi
    ...
    return () => {
      ...
      pronghornApi.removeChannel(presenceChannel);
