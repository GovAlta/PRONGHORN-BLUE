...
import type { Database, Json } from "@/integrations/pronghorn-api/types";
...
export function useRealtimeAudit(projectId: string, sessionId?: string): UseRealtimeAuditReturn {
  ...
  const loadSessionData = useCallback(async (sid: string) => {
    ...
    try {
      const { data: sessions } = await pronghornApi.rpc("get_audit_sessions_with_token", { p_project_id: projectId, p_token: shareToken });
      const currentSession = sessions?.find((s: AuditSession) => s.id === sid);
      if (currentSession) setSession(currentSession);
      
      const { data: entries } = await pronghornApi.rpc("get_audit_blackboard_with_token", { p_session_id: sid, p_token: shareToken });
      setBlackboardEntries(entries || []);
      
      const { data: cells } = await pronghornApi.rpc("get_audit_tesseract_cells_with_token", { p_session_id: sid, p_token: shareToken });
      setTesseractCells(cells || []);

      // Load graph data
      const { data: nodes } = await pronghornApi.rpc("get_audit_graph_nodes_with_token", { p_session_id: sid, p_token: shareToken });
      setGraphNodes((nodes as AuditGraphNode[]) || []);

      const { data: edges } = await pronghornApi.rpc("get_audit_graph_edges_with_token", { p_session_id: sid, p_token: shareToken });
      setGraphEdges((edges as AuditGraphEdge[]) || []);

      // Load activity stream (increased limit to support longer audits)
      const { data: activities } = await pronghornApi.rpc("get_audit_activity_stream_with_token", { p_session_id: sid, p_token: shareToken, p_limit: 1000 });
  ...
  const createSession = useCallback(async (params: CreateSessionParams): Promise<AuditSession | null> => {
    ...
    const { data, error } = await pronghornApi.rpc("insert_audit_session_with_token", {
  ...
  const updateSessionStatus = useCallback(async (sid: string, status: string) => {
    if (!shareToken) return;
    await pronghornApi.rpc("update_audit_session_with_token", { p_session_id: sid, p_token: shareToken, p_status: status });
  }, [shareToken]);

  const writeToBlackboard = useCallback(async (entry: WriteBlackboardParams) => {
    if (!shareToken) return;
    await pronghornApi.rpc("insert_audit_blackboard_with_token", {
  ...
  const writeTesseractCell = useCallback(async (cell: WriteTesseractCellParams) => {
    if (!shareToken) return;
    await pronghornApi.rpc("upsert_audit_tesseract_cell_with_token", {
  ...
  const pruneOrphanNodes = useCallback(async (sid: string): Promise<number> => {
    ...
    for (const nodeId of orphanNodeIds) {
      const { error } = await pronghornApi.rpc("delete_audit_graph_node_with_token", {
        p_node_id: nodeId,
        p_token: shareToken,
      });
      if (!error) {
  ...
  const saveAuditData = useCallback(async (
    ...
    try {
      ...
      if (data.nodes && data.nodes.length > 0) {
        const { data: nodeCount, error: nodeErr } = await pronghornApi.rpc("insert_audit_graph_nodes_batch_with_token", {
          p_session_id: sid,
          p_token: shareToken,
      ...
      if (data.edges && data.edges.length > 0) {
        const { data: edgeCount, error: edgeErr } = await pronghornApi.rpc("insert_audit_graph_edges_batch_with_token", {
          p_session_id: sid,
          p_token: shareToken,
      ...
      if (data.tesseractCells && data.tesseractCells.length > 0) {
        const { data: cellCount, error: cellErr } = await pronghornApi.rpc("insert_audit_tesseract_cells_batch_with_token", {
          p_session_id: sid,
          p_token: shareToken,
      ...
      if (data.activityLog && data.activityLog.length > 0) {
        const { data: actCount, error: actErr } = await pronghornApi.rpc("insert_audit_activity_batch_with_token", {
          p_session_id: sid,
          p_token: shareToken,
      ...
      if (data.vennResult || data.markComplete) {
        const { error: vennErr } = await pronghornApi.rpc("update_audit_session_venn_with_token", {
          p_session_id: sid,
          p_token: shareToken,
