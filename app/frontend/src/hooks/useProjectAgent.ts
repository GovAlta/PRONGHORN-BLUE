...
export function useProjectAgent(
  ...
  const loadAgentConfig = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiClient.post<{ data: any[], error: any }>("/api/v1/rpc/get_project_agent_with_token", {
      ...
      if (response.data && response.data.length > 0) {
        ...
        const customConfig = response.data[0];
  ...
  const saveAgentConfig = useCallback(async (definition: AgentDefinition): Promise<boolean> => {
    ...
    try {
      ...
      const response = await apiClient.post<{ data: any, error: any }>("/api/v1/rpc/upsert_project_agent_with_token", {
        p_project_id: projectId,
        p_token: shareToken,
        p_agent_type: agentType,
  ...
  const resetToDefault = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const response = await apiClient.post<{ data: any, error: any }>("/api/v1/rpc/delete_project_agent_with_token", {
        p_project_id: projectId,
        p_token: shareToken,
        p_agent_type: agentType,
      });
